...
public class ChiTietHoaDon extends javax.swing.JFrame {
     ...
     */
    DefaultTableModel tbm_chitiet = new DefaultTableModel();

    public ChiTietHoaDon(int idHoaDon) {
        initComponents();
        this.setLocationRelativeTo(null);
        this.setResizable(false);
        String[] tieudechitiet = new String[]{"ID món", "Tên Món", "Số Lượng", "Đơn Giá", "Thành Tiền"};
        tbm_chitiet.setColumnIdentifiers(tieudechitiet);
        tableChiTiet.setRowHeight(30);

        LoadTable(idHoaDon);
    }

    public void LoadTable(int idOrder) {
        tbm_chitiet.setRowCount(0);
        ArrayList<Order_Detail> dsPhieuNhap = QuanLyHoaDonDAO.GetDetail(idOrder);

        for (Order_Detail item : dsPhieuNhap) {

            Object[] rowData = {item.getIdMonan(), item.getTenMonan(), item.getSoLuong(), item.getDonGia(), item.getThanhTien()};
            tbm_chitiet.addRow(rowData);

        }
        tableChiTiet.setModel(tbm_chitiet);
    }
